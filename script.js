...
function loadExample() {
    ...
    if (example && editor) {
        ...
        document.getElementById('output').innerHTML = '<span class="placeholder">Your output will appear here...</span>';
    ...
    try {
        ...
        const response = await fetch('https://api.jdoodle.com/v1/execute', {
            ...
            body: JSON.stringify({
                ...
                clientSecret: 'af13f89eadc62f9c2d8a1a64f33e91a0a34d3e8f0b2c4d5e6f7a8b9c0d1e2f3a4', // Demo key
                script: code,
                stdin: input,
                language: 'c',
                versionIndex: '5'
        ...
        if (result.error || (result.output && result.output.toLowerCase().includes('error'))) {
            // Compilation Error
            outputDiv.textContent = result.output || result.error || 'Compilation error occurred';
        ...
        try {
            ...
            if (wandboxResult.compiler_error || wandboxResult.signal) {
                outputDiv.textContent = wandboxResult.compiler_error || wandboxResult.program_error || 'Compilation error';
                outputDiv.className = 'output-area error';
                statusDiv.textContent = '❌ Compilation Error';
                statusDiv.className = 'status error';
            } else {
